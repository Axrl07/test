...
import (
	...
	sistema "main/comandos/adminSistemaDeArchivos"
	usuarios "main/comandos/adminUsuarios"
	reportes "main/comandos/reportes"
	"strings"
)

func Analizar(cadena string) string {
	// respuesta final
	respuesta := ""
	...
	case "mkfs":
		if len(parametros) > 1 {
			respuesta = sistema.Mkfs(parametros[1:])
		} else {
			msj := "ERROR: Faltan parametros en el comando mkfs"
			fmt.Println(msj)
			respuesta += msj
		}
	case "cat":
	...
	case "journaling":
		if len(parametros) > 1 {
			respuesta = sistema.Journaling(parametros[1:])
		} else {
			msj := "ERROR: Faltan parametros en el comando chmod"
			fmt.Println(msj)
			respuesta += msj
	...
	case "recovery":
		if len(parametros) > 1 {
			respuesta = sistema.Recovery(parametros[1:])
		} else {
			msj := "ERROR: Faltan parametros en el comando chmod"
			fmt.Println(msj)
			respuesta += msj
	...
	case "loss":
		if len(parametros) > 1 {
			respuesta = sistema.Loss(parametros[1:])
		} else {
			msj := "ERROR: Faltan parametros en el comando chmod"
			fmt.Println(msj)
			respuesta += msj
